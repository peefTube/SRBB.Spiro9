...
*/
    public class GizmoUtility
    {
            ...
            foreach (GameObject go in objects)
            {
                ...
                foreach (var renderer in renderers)
                {
                    if (renderer.bounds.IntersectRay(ray))
                    {
                        ...
                        if (mf != null && mf.sharedMesh != null && mf.sharedMesh.isReadable && MeshRaycast(mf.transform, mf.sharedMesh, ray, out hit))
                        {
                ...
                {
                    if(collider.Raycast(ray, out RaycastHit rhit, 500))
                    {
                        if(rhit.distance < best)
                        {
                            best = rhit.distance;
                            obj = go;
                        }
        ...
        public static bool MeshRaycast(Transform obj, Mesh mesh, Ray ray, out BB_RaycastHit hit)
        {
            ...
            {
                a = obj.TransformPoint(vertices[triangles[i + 0]]);
                b = obj.TransformPoint(vertices[triangles[i + 1]]);
                c = obj.TransformPoint(vertices[triangles[i + 2]]);

                if (BB_Geometry.RayIntersectsTriangle(ray, a, b, c, Culling.Front, out dist, out point))
                {
                    hit = new BB_RaycastHit();
                    hit.point = point;
                ...
                //if((dist = IntersectRayTriangle(ray, a, b, c)) < 400f)
                //{
                //    hit = new BB_RaycastHit();
                //    hit.point = (a + b + c) / 3;
                //    hit.distance = dist;
         ...
         */
        public static float GetHandleSize(Camera cam, Vector3 position)
        {
            if (!cam) return 1f;
